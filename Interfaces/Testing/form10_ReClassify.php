...
echo '<form action="'.$PAGE_LINK ."&a=208&Initiative={$Initiative}&Classification={$Classification->id}".'" method="post" id="myform" name="myform">';
...
Order By classifications.left_key", $Classification->Classification);
	...
	$mess->Load ( $res );
	foreach ( $mess as $row ) {
		$modalSelect = new ModalBox ( "{$PAGE_LINK}&f=10&Initiative={$Initiative}&Classification={$row->Link}&Selected=8", i_Classify);
...
if ($row->attr) {
	...
	$modalChange = new ModalBox ( "{$PAGE_LINK}&f=10&Initiative={$Initiative}&Classification={$Classification->id}", i_Classify);
