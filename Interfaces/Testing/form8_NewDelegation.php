...
echo '<form action="'.$PAGE_LINK ."&a=401&id={$id}&toMember={$toMember}&byClassification={$byClassification}&delegationType={$delegationType}".'" method="post" id="myform" name="myform">';
echo '<table width="100%" class="NoBorderTable">';
if ($delegationType == '') {
	$modalActive = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$toMember}&byClassification={$byClassification}&delegationType=1", i_NewDelegation,0,0,0,true);
	$modalPassive = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$toMember}&byClassification={$byClassification}&delegationType=2", i_NewDelegation,0,0,0,true );
	...
	$modalChange = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$toMember}&byClassification={$byClassification}", i_NewDelegation,0,0,0,true );
	echo '<tr><td>' . i_DelegationType . ':</td>';
...
if ($toMember == '') {
	...
	foreach ( $members as $row ) {
		$modalSelect = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$row->link}&byClassification={$byClassification}&delegationType={$delegationType}", i_NewDelegation,0,0,0,true );
	...
	$modalChange = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&byClassification={$byClassification}&delegationType={$delegationType}", i_NewDelegation,0,0,0,true );
	$member = new Members ( );
...
Order By classifications.left_key" );
	...
	$mess->Load ( $res );
	foreach ( $mess as $row ) {
		$modalSelect = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$toMember}&byClassification={$row->Link}&delegationType={$delegationType}", i_NewDelegation,0,0,0,true );
	...
	$modalChange = new ModalBox ( "{$PAGE_LINK}&f=8&id={$id}&toMember={$toMember}&delegationType={$delegationType}", i_NewDelegation,0,0,0,true );
...
//$SaveLink = '<a href="' . $PAGE_LINK . "&a=401&id={$id}&toMember={$toMember}&byClassification={$byClassification}&delegationType={$delegationType}" .'" onclick="Form.serialize()">' . i_Save . '</a>';
